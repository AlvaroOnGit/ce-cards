...
export async function getCardsBySet(setCode: string, page: number = 1): Promise<{ cards: Card[], hasMore: boolean, nextUrl: string | null }> {
    const url = `https://api.scryfall.com/cards/search?q=set:${setCode}&page=${page}`;
    const res = await fetch(url);
    const data = await res.json();

    const cards: Card[] = data.data.map((c: any) => ({
        id:            c.id,
        name:          c.name,
        type:          c.type_line,
